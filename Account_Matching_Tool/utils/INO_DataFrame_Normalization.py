...
def account_table_clean_updated(input_df, source) -> DataFrame:
    ...
    if source == "sf":
        ...
        if "DHCID" not in input_df:
            ...
            input_df.DHCID = input_df.DHCID.apply(
                lambda dhc: dhc[:-2] if dhc[-2:] == ".0" else dhc
            )
        if "DHCNetworkID" not in input_df:
            ...
            input_df.DHCNetworkID = input_df.DHCNetworkID.apply(
                lambda dhcn: dhcn[:-2] if dhcn[-2:] == ".0" else dhcn
            )
    ...
    if "Phone" not in input_df:
        ...
        input_df.Phone = input_df.Phone.apply(
            lambda phone: phone[:-2] if phone[-2:] == ".0" else phone
        )
        input_df.Phone = input_df.Phone.apply(
            lambda x: x.replace("(", "")
            .replace("-", "")
    ...
    if "NPI__c" in input_df:
        ...
        input_df.NPI__c = input_df.NPI__c.apply(
            lambda npi: npi[:-2] if npi[-2:] == ".0" else npi
        )
    elif "NPI" in input_df:
        input_df.NPI = input_df.NPI.astype(str)
        input_df.NPI = input_df.NPI.apply(
            lambda npi: npi[:-2] if npi[-2:] == ".0" else npi
        )
    ...
    if "Ext" in input_df:
        ...
        input_df.Ext = input_df.Ext.apply(
            lambda ext: ext[:-2] if ext[-2:] == ".0" else ext
        )
    ...
    if "TaxID" in input_df:
        ...
        input_df.Tax_ID__c = input_df.Tax_ID__c.apply(
...
def regex_account_name(input_df, column_name) -> DataFrame:
    ...
    input_df["NormalizedName"] = (
        input_df["NormalizedName"].str.replace(r"[^\w\s+]", " ", regex=True).str.strip()
    )
...
def normalize_street(input_df, column_name) -> DataFrame:
    if "AddressType" in input_df.columns:
        ...
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
                : name.find(" ste ") if name.find(" ste ") > 0 else len(name) + 1
            ]
        )
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
                : name.find("apt.") if name.find("apt.") > 0 else len(name) + 1
            ]
        )
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
        ...
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
                : name.find("appt") if name.find("appt") > 0 else len(name) + 1
            ]
        )
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
                : name.find("no.") if name.find("no.") > 0 else len(name) + 1
            ]
        )
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
                : name.find(" unit ") if name.find(" unit ") > 0 else len(name) + 1
            ]
        )
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
        ...
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
                : name.find("suite") if name.find("suite") > 0 else len(name) + 1
            ]
        )
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
                : name.find("number") if name.find("number") > 0 else len(name) + 1
            ]
        )
        input_df["NormAddress"] = input_df.NormAddress.apply(
            lambda name: name[
...
def normalize_suite(input_df, column_name) -> DataFrame:
    ...
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find(" ste ") if name.find(" ste ") > 0 else 0 : len(name) + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find("apt.") if name.find("apt.") > 0 else 0 : len(name) + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
    ...
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find("appt") if name.find("appt") > 0 else 0 : len(name) + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find("no.") if name.find("no.") > 0 else 0 : len(name) + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find(" unit ") if name.find(" unit ") > 0 else 0 : len(name) + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find("appartment") if name.find("appartment") > 0 else 0 : len(name)
            + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
    ...
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find("suite") if name.find("suite") > 0 else 0 : len(name) + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
        lambda name: name[
            name.find("number") if name.find("number") > 0 else 0 : len(name) + 1
        ]
    )
    input_df["NormAddressSuite"] = input_df.NormAddressSuite.apply(
