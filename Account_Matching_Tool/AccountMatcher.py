...
def reshape_addresses(input_df) -> pd.DataFrame:
    if "ShippingStreet" in input_df.columns and "BillingStreet" in input_df.columns:
        print(
            "Shipping and Billing Addresses Exist.\nChecking if they're the same address."
        )  # noqa: E501
        shiptup = (
            "ShippingStreet",
            "ShippingCity",
        ...
        address_match = (
            (input_df[shiptup[0]] == input_df[billtup[0]]).all()
            and (input_df[shiptup[1]] == input_df[billtup[1]]).all()
            and (input_df[shiptup[2]] == input_df[billtup[2]]).all()
            and (input_df[shiptup[3]] == input_df[billtup[3]]).all()
        )
        if address_match:
            ...
            merged_df = merged_df.sort_values(
                ["AccountName", "AddressType"], ascending=[True, True]
            )
            del ship_df, bill_df
            # return merged df
            return merged_df
...
def match_columns(input_df) -> pd.DataFrame:
    ...
    input_df["Match4"] = input_df.Address + "|" + input_df.City + "|" + input_df.State
    return input_df
...
def new_name_matching(input_df) -> None:
    ...
    for col in coefficient_columns:
        match_col = f"{col}_sf"
        input_df.loc[
            non_name_matches & (input_df[match_col] == input_df[f"{col}_in"]),
            "MatchScore",
        ] += 10
        input_df.loc[
            non_name_matches & (input_df[match_col] == input_df[f"{col}_in"]),
...
def address_scorer(input_df) -> None:
    ...
    CS_match = (input_df[f"{co_col[0]}_sf"] == input_df[f"{co_col[0]}_in"]) & (
        input_df[f"{co_col[1]}_sf"] == input_df[f"{co_col[1]}_in"]
    )
    ACS_match = (
    ...
    A_match = input_df[f"{co_col[3]}_sf"] == input_df[f"{co_col[3]}_in"]
...
def final_score_eval(input_df) -> None:
    if "LOB_sf" in input_df:
        ...
        input_df["LOBMatch"] = 1
    input_df["FullNameMatch"] = (
        input_df.NormalizedName_sf == input_df.NormalizedName_in
    ).astype(int) + 1
...
def match_list_maker(input_df) -> None:
    if "FuzzyNameMatch" in input_df:
        input_df["FuzzyName"] = (
            (input_df.FuzzyNameMatch >= 80)
            .astype(str)
            .replace("True", ",fuzzyname")
            .replace("False", "")
        )
    if "FuzzyAddressMatch" in input_df:
        input_df["FuzzyAddress"] = (
            (input_df.FuzzyAddressMatch >= 80)
            .astype(str)
            .replace("True", ",fuzzyaddress")
            .replace("False", "")
        )
    input_df["Phone"] = (
        (input_df.Phone_sf == input_df.Phone_in)
        .astype(str)
        .replace("True", ",Phone")
        .replace("False", "")
    )
    ...
    if "NPI_in" in input_df:
        input_df["NPI"] = (
            (input_df.NPI_in == input_df.NPI_sf)
            .astype(str)
            .replace("True", ",NPI")
            .replace("False", "")
        )
    if "LOB_in" in input_df:
        input_df["PLOB"] = (
            (input_df.LOB_sf == input_df.LOB_in)
            .astype(str)
            .replace("True", ",PLOB")
            .replace("False", "")
        )
    if "Taxonomy" in input_df:
        input_df["Taxonomy"] = (
            (input_df.Taxonomy == input_df.Taxonomy_Primary)
            .astype(str)
            .replace("True", ",Taxonomy")
            .replace("False", "")
        )
    if "TaxID_in" in input_df:
        input_df["TaxID"] = (
            (input_df.TaxID_in == input_df.TaxID_sf)
            .astype(str)
            .replace("True", ",TaxID")
            .replace("False", "")
        )
    ...
    input_df.MatchList.apply(
        lambda matchtype: matchtype[1:] if len(matchtype) > 0 else matchtype
    )
...
def tie_breaker(input_df) -> pd.DataFrame:
    ...
    date_prep = pd.merge(
        input_df, ungrouped["IsUltimateParentMatch"], left_index=True, right_index=True
    )
    # Max Date by Input Account Name Group Match
    date_prep["MaxDate"] = date_prep.groupby(
        "AccountName_in"
    ).LastModifiedDate.transform("max")
    date_prep["DateCheck"] = date_prep["LastModifiedDate"] == date_prep["MaxDate"]

    status_values = [
        date_prep["AccountStatusScore"].eq(1),
        date_prep["AccountStatusScore"].eq(2),
        date_prep["AccountStatusScore"].eq(3),
        date_prep["AccountStatusScore"].eq(4),
    ...
    date_prep.IsUltimateParentMatch = date_prep.IsUltimateParentMatch.replace(
        True, "25"
    ).replace(False, "0")
...
def account_match_group_evaluation(input_df) -> pd.DataFrame:
    ...
    df_matchCount = pd.DataFrame(
        data=series_MatchCount, columns=["Count", "AccountName_in"]
    )
    df_matchCount["Count"] = df_matchCount["AccountName_in"]
    match_ready_df = pd.merge(
        input_df,
        df_matchCount["Count"],
        how="right",
        left_on="AccountName_in",
    ...
    match_ready_df["Rank"] = (
        match_ready_df.groupby("AccountName_in")["MatchScore"]
        .rank(ascending=False, method="average")
        .to_frame()
    )  # type: ignore
    match_ready_df["eval_result"] = match_ready_df.groupby("AccountName_in")[
        "MatchScore"
    ].transform(lambda x: x.sum() / match_ready_df["Count"])
    ...
    master_conditions = [
        ...
        (
            match_ready_df["Rank"].eq(1)
            & match_ready_df["eval_result"].ne(match_ready_df["MatchScore"])
        )
        | (match_ready_df["Rank"].eq(1) & match_ready_df["Count"].eq(1)),
    ...
    match_ready_df["BestMatch"] = np.select(
        master_conditions, master_choices, default="Merge"
    )
...
def accountmatcher(name, fuzzy=False, all_matches=False) -> None:
    ...
    start = time1_start
    sf_account_df = sqe.sql_extract(
        query=sfq.view_query, server="MSPBIDB02", db="Analytics", conn_type=""
    )
    stop = timeit.default_timer()
    ...
    if fuzzy is True:
        ...
        basematches_df["FuzzyNameMatch"] = basematches_df.apply(
            lambda x: rapidfuzz.fuzz.token_sort_ratio(
                x.AccountName_in, x.AccountName_sf
            ),
            axis=1,
        ).to_list()
        stop = timeit.default_timer()
        print(
            "Fuzzy Name Matching Completed. Time: {:.2f} seconds".format(stop - start)
        )

        # Start Fuzzy Address Matching
        start = timeit.default_timer()
        basematches_df["FuzzyAddressMatch"] = basematches_df.apply(
            lambda x: rapidfuzz.fuzz.token_sort_ratio(x.Address_in, x.Address_sf),
            axis=1,
        ).to_list()
        stop = timeit.default_timer()
        print(
            "Fuzzy Address Matching Completed. Time: {:.2f} seconds".format(
                stop - start
            )
        )
    ...
    print(
        "Match and Scoring Processing Time: {:.2f} seconds".format(
    ...
    matches_for_tieBreaking = review_group[
        review_group.groupby("AccountName_in")["AccountName_in"].transform("count") > 5
    ]
    ...
    if matches_for_tieBreaking.empty:
        ...
        passable_reviews = review_group[
            review_group.groupby("AccountName_in")["AccountName_in"].transform("count")
            <= 5
        ]
        ...
        re_evaluated.sort_values(
            ["AccountName_in", "Rank"], ascending=[True, True], inplace=True
        )
    ...
    df_clean.sort_values(
        ["MatchScore", "AccountStatusScore", "ParentScore"],
    ...
    df_final.sort_values(
        ["MatchScore", "AccountName", "AccountStatusScore", "ParentScore"],
    ...
    if all_matches is True:
        ...
        fil_out = (df_final["MatchScore"] >= 50) & (
            df_final["BestMatch"] == "Review Account Group"
        )
    ...
    print(
        "Account Matching Completed. Total Time: {:.2f} seconds".format(
            stop - time1_start
        )
    )
