...
df["A_Name_Match_C_Em_Dom"] = df.apply(lambda x: x.AccountName.replace(" ","").lower()
                                        in x.contactemaildomain, axis=1, 
                                        result_type="expand").to_frame()
#Max Date
...
df["MatchLnFn"] = df.groupby(["email","LastName","FirstName"])["contactid"].transform(
    lambda x : x.size)
df["MatchLN"] = df.groupby(["email", "LastName"])["contactid"].transform(
    lambda x :x.size)
df["MatchLn3Fn"] = df.groupby(["email","LastName","First3Name"])["contactid"].transform(
    lambda x :x.size)
df["MatchTitle"] = df.groupby(["email","Title"])["contactid"].transform(
    lambda x: x.size)
df["MatchPhone"] = df.groupby(["email","ContactPhone"])["contactid"].transform(
    lambda x: x.size)
...
dupeEval = [
df["EmCount"].eq(2) & (df["MatchLnFn"].eq(1) & df["MatchLN"].eq(1) & 
                       df["MatchLn3Fn"].eq(1)),
df["EmCount"].eq(3) & (df["MatchLnFn"].eq(1) & df["MatchLN"].eq(1) & 
                       df["MatchLn3Fn"].eq(1)),
df["EmCount"].gt(df["MatchLnFn"]) & df["EmCount"].gt(
    df["MatchLn3Fn"]) & df["EmCount"].gt(df["MatchLN"]),
...
df["Rank"] = df.groupby("email")["points_total"].rank(
    ascending=False,method="average").to_frame()


df["eval_result"] = df.groupby("email")["points_total"].transform(lambda x: x.sum()/
                                                                  df["EmCount"])
df["Review_Groups"] = df["email"].map(df.groupby("email")
                                      .apply(lambda x: x["Rank"].ne(1).all())) 
#Returns True if all rows in an email group that does not have a Rank of 1
df["Merge_Groups"] = df["email"].map(df.groupby("email")
                                     .apply(lambda x: x["Rank"].eq(1).any()))
 #Returns True if any row in an email group has a Rank of 1
master_conditions = [
    df["Rank"].eq(1) & df["eval_result"].ne(df["points_total"]),
    df["Rank"].ne(1) & df["Merge_Groups"].eq(True),
    df["eval_result"].eq(df["points_total"]) | (df["Rank"] % 1).ne(0)
      | df["Review_Groups"].eq(True)  
]
...
'MatchPhone','points','points2','eval_result',"Review_Groups","Merge_Groups"],
axis = 1,inplace=True)


df  = df.reindex(columns=['Data Load Status','Data Load Date','Dupes','points_total',
                        'Rank','MasterPicker','Notes','Researcher','Research Status',
                        'Research Date','Not a Duplicate?','Master Record?','Include?',
                        'Missing Master?','OppCount','Type','email',
                        'contactemaildomain','AccountName','ParentName','UltParentName',
                        'accntwebdomain','accntemaildomain','ContactName','ContactPhone',
                        'AccountPhone','Account_Status__c','Contact_Status__c','CreatedDate',
                        'UsingPartnerCommunity','ContactZipCode','AccountNumber','AccountZipCode',
                        'UPSameAsAccount','EmCount','FirstName','LastName','Title',
                        'accountid','ParentID','UltParentID','contactid'])
...
time_total = time_connect_delta + time_clean_delta 
+ time_eval_delta + time_export_delta 
