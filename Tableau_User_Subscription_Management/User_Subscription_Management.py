...
sql_schema = "Tableau"
sql_usp = "uspTableauAPIDataMaintain"
sql_view = "View_TEST"
...
def fetch_tableau_data(conn):
    ...
    ["createdAt", "updatedAt", "type", "frequency", "nextRunAt", "state", "priority"], axis=1)
    t_schedules_final = t_schedules_final.rename(
    columns={"id": "ScheduleID", "name": "ScheduleName"})

    t_groups_final = t_groups.rename(
    columns={"id": "GroupID", "name": "GroupName"})
    ...
    t_views_final = t_views_final.rename(
    columns={"id": "ViewID", "name": "ViewName"})

    t_projects_final = t_projects.sort_values(by=['parentProjectId'])
    t_projects_final.drop(["owner", "description", "createdAt", "updatedAt",
                        "contentPermissions", "controllingPermissionsProjectId"], axis=1, inplace=True)
    t_projects_final.rename(columns={
                        "id": "ProjectID", "name": "ProjectName", "parentProjectId": "ParentProjectID"}, inplace=True)

    t_workbook["ProjectID"] = t_workbook["project"].apply(
    lambda x: x.get('id'))
    t_workbook["ProjectName"] = t_workbook["project"].apply(
    lambda x: x.get('name'))
    t_workbook.rename(columns={"id": "WorkbookID", "name": "WorkbookName", "description": "Description",
                            "createdAt": "CreatedAt", "updatedAt": "UpdatedAt", "size": "Size", "defaultViewId": "DefaultViewID"}, inplace=True)
    t_workbook_final = t_workbook[["WorkbookID", "WorkbookName", "ProjectID",
                                "ProjectName", "Description", "DefaultViewID", "Size", "CreatedAt", "UpdatedAt"]]

    df_tuple = (t_views_final, t_projects_final, t_groups_final,
            t_schedules_final, t_subscriptions, t_workbook_final)
    return df_tuple
...
def sql_data_maintain(stored_procedure):
    """
    The sql_data_maintain function executes a stored procedure on the SQL Server database.

    Args:    
        stored_procedure (string): Name of the stored procedure to call in sql server
    Returns:
        Nothing
    """
...
def subscriptions_pull(conn):
    ...
    subs_df.rename(columns={"id": "subscription_id",
                   "content_id": "view_id"}, inplace=True)
    final_subs = subs_df[["subscription_id",
                          "view_id", "user_id", "user_name"]]
    return final_subs


def create_user_subscription(conn, scheduleid, contentid, contenttype, sub_subject, my_message, userid, attach_pdf_flag, attach_image_flag, pdf_page_orientation, pdf_page_size, send_view_if_empty_flag):
    ...
    Returns:
        Nothing
    """
    create = conn.create_subscription(schedule_id=scheduleid,
...
def create_missing_subscriptions(input_dataframe, tableau_subscriptions, connection):
    ...
    #       GET subscriptions FOR ViewPathAndName
        subscriptions_filtered = tableau_subscriptions[tableau_subscriptions["content_id"] == ViewID]
    ...
    #       FOR EACH GroupName IN Analytics.Tableau.GroupSubscription_TEST {
        for groupID in input_dataframe["GroupID"].unique():
            ...
            get_missing_subscriptions = licensed_users.join(subscriptions_filtered.set_index(
                "user_id"), on="UserID", how="left", lsuffix="_l", rsuffix="_tab")
            ...
            if len(missing_subscriptions) == 0:
                ...
                for row in missing_subscriptions.itertuples():
                    ...
                    scrp_subject = input_dataframe["SubscriptionSubject"].to_string(
                        index=False)
                    scrp_id = input_dataframe["ScheduleID"].to_string(
                        index=False)
                    scrp_message = input_dataframe[""]  # Needs to be defined
                    scrp_username = row.fullName
                    scrp_user = row.UserID
                    scrp_image = input_dataframe["AttachImage"].to_string(
                        index=False)
                    scrp_pdf = input_dataframe["AttachPDF"].to_string(
                        index=False)
                    ...
                    print("Creating subscription for {} to {}".format(
                        scrp_username, scrp_subject))

                    create_user_subscription(conn=connection,
                                             scheduleid=scrp_id,
                                             contentid=ViewID,
                                             contenttype="view",
...
def delete_unlicensed_user_subscriptions(tableau_subscriptions, connection):
    ...
    deletable_subscriptions = tableau_subscriptions[tableau_subscriptions["user_id"].isin(
        all_unlicensed_users["id"])]
    ...
    for row in deletable_subscriptions.itertuples():
        ...
        print("Deleting subscription to '{}' for unlicensed user: {}".format(
            subscription_subject, delete_users))
        delete_user_subscription(subscription_id)
...
def main():
    ...
    sql_tableau_data = sql_tableau_publication.join(
        sql_tableau_group_subscription.set_index("PublicationID"), on="PublicationID", how="right")
